...
{
	internal sealed class TwilioSenderVerificationProviderClient(TwilioClientOptions _twilioClientOptions) : SenderVerificationChannelProviderRestClient(null)
	{
		...
		public override async Task<ISenderVerificationValidationResult> ConfirmSenderVerificationAsync(ISenderVerificationContext senderVerificationContext, string code, string? token = null)
		{
			...
			var result = await VerificationCheckResource.CreateAsync(new CreateVerificationCheckOptions(properties.VerificationSid)
			{
				...
				Code = code
			}, new TwilioHttpClient(HttpClient, _twilioClientOptions));
		...
		public override async Task<IReadOnlyCollection<IInitiateSenderVerificationResult>> InitiateSenderVerificationAsync(ISenderVerificationContext senderVerificationContext)
		{
			...
			var verification = await VerificationResource.CreateAsync(
				...
				pathServiceSid: properties.VerificationSid, client: new TwilioHttpClient(HttpClient, _twilioClientOptions));
		...
		public override Task<ISenderVerificationStatusResult> IsSenderVerifiedAsync(ISenderVerificationContext senderVerificationContext, string? token = null)
		{
